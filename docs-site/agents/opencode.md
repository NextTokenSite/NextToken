...
1. 按 [创建 API Key 教程](https://docs.nexttoken.online/platform/create-apikey) 生成 API Key。
...
```json
{
  ...
  "provider": {
    "nexttoken": {
      ...
      "name": "nexttoken",
      "options": {
        "baseURL": "https://nexttoken.online/v1",
...
- `npm` 指定 OpenCode 使用 `@ai-sdk/openai-compatible` 接入 `https://nexttoken.online/v1`。
